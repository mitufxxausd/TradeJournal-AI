      ...
      <CardContent className="p-4 flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Your browser does not support voice recording. Try using Chrome, Edge, or Safari for full functionality.
          </p>
        </div>
      </CardContent>
    </Card>
...
export default function AIVoiceNotes() {
  ...
  const handleStartRecording = async () => {
    if (!isMediaSupported) {
      toast.info("Voice recording is not supported in this browser.");
      return;
    }
    try {
  ...
  const handleSaveRecording = useCallback(async () => {
    ...
    setIsUploading(true);
    let persistentUrl = recorder.audioUrl;
    ...
    try {
      ...
      console.warn("Cloudinary voice upload failed, using local blob:", uploadErr);
      // Fall back to blob URL - note: this won't survive refresh but at least works for current session
    } finally {
  ...
  const handleStartEditTranscript = (note: VoiceNote) => {
    setEditingTranscript(note.id);
    setEditTranscriptValue(note.transcript);
  };

  const handleSaveTranscript = (noteId: string) => {
    setVoiceNotes((prev) =>
      prev.map((n) =>
        n.id === noteId
          ? { ...n, transcript: editTranscriptValue, transcribed: editTranscriptValue.length > 0 }
          : n
      )
    );
